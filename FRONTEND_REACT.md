...
Xato:
```json
{ "success": false, "statusCode": 400, "message": "...", "errors": ["..."], "timestamp": "...", "path": "..." }
```
> Validatsiya xatolari `errors` massivida (forma maydonlariga bog'lash uchun).

### Sahifalangan ro'yxat (`data`)
```json
{ "items": [ ], "meta": { "total": 42, "page": 1, "limit": 20, "totalPages": 3, "hasNextPage": true, "hasPrevPage": false } }
```
...
```json
{
  ...
  "sales":        { "paidCount": 0, "revenue": 0, "revenueToday": 0, "revenueThisWeek": 0, "revenueThisMonth": 0 },
...
- **`targetType`** (`BOOK | COLLECTION | URL`) ga mos **bitta** target maydoni
  ...
  tekshiriladi).
- **Rasm**: avval banner yaratiladi (`POST /banners`), so'ng `POST /banners/:id/image`.
...
POST /books
{
  ...
  "authorId":      "<uuid>",            // bitta muallif (ixtiyoriy)
  "categoryIds":   ["<uuid>"],          // N—N
  "collectionIds": ["<uuid>"],          // N—N
  "editions": [                          // kamida bitta format
    { "format": "AUDIO", "narrator": "Akmal Po'latov", "durationSeconds": 18540 },
...
- **`authorId`** — bitta muallif (massiv emas). `categoryIds`/`collectionIds` — N—N (massiv).
- **`editions`** — kamida 1 ta. Har format bir marta (ikkita `AUDIO` → `400`).
  Audio uchun `narrator`/`durationSeconds`, e-kitob uchun `pageCount`.
...
- **`authorId`/`categoryIds`/`collectionIds`** — mavjud ID'lar bo'lishi shart
...
`author` (bitta), `categories[]`, `collections[]`, `editions[]`, `status` bilan).
...
- Cheklovlar: audio ≤ **500MB**, e-kitob ≤ **100MB**, preview ≤ **50MB**;
...
```ts
async function uploadContent(editionId: string, file: File, kind: 'CONTENT' | 'PREVIEW') {
  const { data } = await api.post(`/editions/${editionId}/upload-url`, {
    kind, mime: file.type, sizeBytes: file.size,
  });
  const { uploadUrl } = data.data;
  await fetch(uploadUrl, { method: 'PUT', headers: { 'Content-Type': file.type }, body: file });
  await api.post(`/editions/${editionId}/process?kind=${kind}`);
...
```
POST /notifications/broadcast  { type, title, body, refId? }   → navbatga qo'yiladi (background)
```
...
- [ ] `authorId` (bitta), `categoryIds`/`collectionIds` (massiv) — mavjud ID
